import React, { Component } from 'react';
import { withRouter } from '../../common/with-router';

class Dashboard extends Component {
	render() {
		return (
			<>
				<div className="sidebar">
					<div className="logo">
						<img src="media/dashboard_24dp_FILL0_wght400_GRAD0_opsz24.svg" />
						<h1>Dashboard</h1>
					</div>
					<ul>
						<li className="item">
							<img
								src="media/home_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Home</h3>
						</li>
						<li className="item">
							<img
								src="media/person_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Profile</h3>
						</li>
						<li className="item">
							<img
								src="media/chat_bubble_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Messages</h3>
						</li>
						<li className="item">
							<img
								src="media/history_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>History</h3>
						</li>
						<li className="item">
							<img
								src="media/task_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Tasks</h3>
						</li>
						<li className="item">
							<img
								src="media/groups_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Communities</h3>
						</li>
					</ul>
					<ul>
						<li className="item">
							<img
								src="media/settings_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Settings</h3>
						</li>
						<li className="item">
							<img
								src="media/contact_support_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Support</h3>
						</li>
						<li className="item">
							<img
								src="media/verified_user_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<h3>Privacy</h3>
						</li>
					</ul>
				</div>
				<div className="header">
					<div className="search">
						<div>
							<img
								src="media/search_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
							/>
							<input type="text" className="searchBar" />
						</div>
					</div>
					<div className="noti">
						<div>
							<img
								src="media/notifications_active_24dp_FILL0_wght400_GRAD0_opsz24.svg"
								alt=""
								className="nsymbol"
							/>
							<img
								src="https://i.pravatar.cc/300"
								alt=""
								className="pfp"
							/>
							<a href="https://github.com/Username">Username</a>
						</div>
					</div>
					<div className="about">
						<div>
							<img
								src="https://i.pravatar.cc/300"
								alt=""
								className="bpfp"
							/>
							<div>
								<h4>Hi there,</h4>
								<h1>Username (@Username)</h1>
							</div>
						</div>
					</div>
					<div className="buttons">
						<div>
							<button className="btn">New</button>
							<button className="btn">Upload</button>
							<button className="btn">Share</button>
						</div>
					</div>
				</div>
				<div className="content">
					<div className="projects">
						<h3>Your Projects</h3>
						<div className="cards">
							<div className="card">
								<div className="cardData">
									<a href="https://username.github.io/Calculator/">
										Calculator
									</a>
									<img src="media/calc.png" alt="" />
									<p>
										Final project in Odin project
										foundations. In this project my skills
										to use the basics of HTML, CSS and
										javascript were tested to it's
										potential.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/Calculator">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
							<div className="card">
								<div className="cardData">
									<a href="https://Username.github.io/Etch-a-Sketch/">
										Etch-a-Sketch
									</a>
									<img src="media/etch.png" alt="" />
									<p>
										In Odin project Foundations course, this
										project was given to practice our DOM
										skills. This game is a mix of scribble
										and etch-a-skitch.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/Etch-a-Sketch">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
							<div className="card">
								<div className="cardData">
									<a href="https://Username.github.io/Sign-up-Form/">
										Sign-up Form
									</a>
									<img src="media/project_pic_1.png" alt="" />
									<p>
										This project is given in Odin Project
										intermediate HTML and CSS course. It
										requires good control over forms and
										styling forms.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/Sign-up-Form">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
							<div className="card">
								<div className="cardData">
									<a href="https://Username.github.io/Landing-Page/">
										Landing Page
									</a>
									<img src="media/lp.png" alt="" />
									<p>
										Landing page website created in the
										foundations course of the oding project.
										Created using HTML and CSS.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/Landing-Page">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
							<div className="card">
								<div className="cardData">
									<a href="https://Username.github.io/odin-recipes/">
										Odin recipes
									</a>
									<img src="media/OdinRecipes.png" alt="" />
									<p>
										First project in the Odin project
										Foundations course built with only HTML.
										This is my first project after learning
										HTML.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/odin-recipes">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
							<div className="card">
								<div className="cardData">
									<a href="https://Username.github.io/rock-paper-scissors/">
										Project Name
									</a>
									<img
										src="media/rockPaperScissors.png"
										alt=""
									/>
									<p>
										This project is given in odin project
										foundations section after learning
										Javascript. This is the first project i
										made after learning javascript. Try to
										win if you can.
									</p>
									<div className="links">
										<a href="">
											<img
												src="media/favorite_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="">
											<img
												src="media/share_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
										<a href="https://github.com/Username/rock-paper-scissors">
											<img
												src="media/link_24dp_FILL0_wght400_GRAD0_opsz24.svg"
												alt=""
											/>
										</a>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div className="social">
						<div className="ants">
							<h3>Announcements</h3>
							<div className="scard">
								<div>
									<h4>Intermediate HTML and CSS</h4>
									<p>
										Completion of intermediate HTML and CSS
										course in the odin project after this
										<span>
											<a href="https://github.com/Username/Admin-Dashboard">
												Admin Dashboard
											</a>
										</span>
										project.
									</p>
								</div>
								<div>
									<h4>GSSOC contribution</h4>
									<p>
										Pull request got merged for
										<span>
											<a href="https://github.com/Username/GameZone/tree/main/Games/Etch_a_Sketch_">
												Etch-a-Sketch-2
											</a>
										</span>
										after raising an issue to add a new game
										in the kunjgit{' '}
										<span>
											<a href="https://github.com/kunjgit/GameZone">
												GameZone
											</a>
										</span>
										.
									</p>
								</div>
								<div>
									<h4>TheOdinProject Foundation</h4>
									<p>
										Foundations course completed for the
										odin project. Learned HTML, CSS,
										JAVASCRIPT, GIT, GITHUB.
									</p>
								</div>
							</div>
						</div>
						<div className="trending">
							<h3>Social</h3>
							<div className="scard">
								<div>
									<img src="media/friend.png" alt="" />
									<div>
										<a href="https://github.com/Username">
											@Username
										</a>
										<h6>Github</h6>
									</div>
								</div>
								<div>
									<img
										src="https://media.licdn.com/dms/image/D5603AQFkzXF0m8j6AA/profile-displayphoto-shrink_400_400/0/1718237328047?e=1723680000&v=beta&t=SyzrZpl1v9mGKqN5QPhQA5PVfwMvfQrDpSnAHoO633M"
										alt=""
									/>
									<div>
										<a href="https://www.linkedin.com/in/username/">
											Username
										</a>
										<h6>Linkedin</h6>
									</div>
								</div>
								<div>
									<img
										src="data:image/png;base64,"
										alt=""
									/>
									<div>
										<a href="https://discord.com/channels/@username">
											@Username
										</a>
										<h6>Discord</h6>
									</div>
								</div>
								<div>
									<img
										src="data:image/png;base64,"
										alt=""
									/>
									<div>
										<a href="https://www.reddit.com/user/Material-Tackle-1647/">
											@M-T-1647
										</a>
										<h6>Reddit</h6>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</>
		);
	}
}

export default withRouter(Dashboard);
